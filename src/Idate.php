...
use function Joonika\Idate\datetodate;
use function Joonika\Idate\periodLastDay;

class Idate
{
    public static function idateReady()
    {
        AstCtrl::ADD_HEADER_STYLES_FILES(JK_DOMAIN() . 'assets/idate/jquery.md.bootstrap.datetimepicker.style.css');
        AstCtrl::ADD_FOOTER_JS_FILES(JK_DOMAIN() . 'assets/idate/jquery.md.bootstrap.datetimepicker.js');
        AstCtrl::ADD_FOOTER_JS_FILES(JK_DOMAIN() . 'assets/idate/jquery-clock-timepicker.js');
    }
    ...
    {
        if ($timezone != 'local') self::date_default_timezone_set($timezone);
        if ($h === '') {
            return time();
        } else {
    ...
    static function setDatetime($options = [])
    {
        $option = [
            "name" => "datetime",
            "value" => '',
        ...
        if ($option['targetDateSelector'] != false) {
            $targetDateSelector = "targetDateSelector : '" . $option['targetDateSelector'] . "',";
        } else {
            $targetDateSelector = "";
        }
        ...
        <script>
            ...
            navigator:{
                ...
                disableAfterToday: ' . $option['disableAfterToday'] . ',
    });
        </script>
        ');
    }

    static function datetoint($val, $format = 3, $lang = '0')
    {
        if ($lang == '0') {
            $lang = JK_LANG();
        }
        ...
        if ($format == 3) {
            ...
            if ($lang == 'fa') {
                ...
                if (sizeof($timerday) >= 2) {
                    $totdate = self::jalali_to_gregorian(intval($timerday[0]), intval($timerday[1]), intval($timerday[2]), '/');
                    $value = $totdate;
                }
            } else {
                $value = $val;
            ...
            if ($lang == 'fa') {
                ...
                if (sizeof($timerday2) >= 2) {
                    $totdate = self::jalali_to_gregorian(intval($timerday2[0]), intval($timerday2[1]), intval($timerday2[2]), '/');
    ...
    static function datetodate($val, $formatSource = 3, $langSource = '0', $formatDest = 6, $langDest = "en")
    {
        if ($langSource == '0') {
            $langSource = JK_LANG();
        }

        $value = self::datetoint($val, $formatSource, $langSource);
        ...
        $newVal = self::date_int($format, $value, $langDest);
        return $newVal;
    }

    static function dateToDateRange($val, $formatSource = 3, $langSource = '0', $formatDest = 6, $langDest = "en")
    {
        if ($langSource == '0') {
            $langSource = JK_LANG();
        }
        ...
        $valueFrom = self::datetoint($from, $formatSource, $langSource);
        if (is_numeric($formatDest)) {
            if ($formatDest == 3) {
                $format = "Y/m/d";
            } else {
        ...
        $newValFrom = self::date_int($format, $valueFrom, $langDest);

        $valueTo = self::datetoint($to, $formatSource, $langSource);
        if (is_numeric($formatDest)) {
            if ($formatDest == 3) {
                $format = "Y/m/d";
            } else {
        ...
        $newValTo = self::date_int($format, $valueTo, $langDest);

        return [$newValFrom, $newValTo];
    }

    static function date_int($format, $int = '', $lang = '0')
    {
        if ($int == '') {
            $int = time();
        }
        ...
        if ($lang == "0" && function_exists("JK_LANG")) {
            $lang = JK_LANG();
        }
        ...
        if ($lang == 'fa') {
            return self::tr_num(self::jdate($format, $int));
        } else {
            return date($format, $int);
        }
    }

    static function field_date($options = [])
    {
        global $data;
        $return = '';
        $option = [
        ...
        if ($option['value'] != "") {
            if ($option['forceVal'] != true) {
                $value = self::date_int($format, $option['value']);
            } else {
                $value = $option['value'];
            }
        } else {
        ...
        self::setDatetime([
            ...
            "targetDateSelector" => $option['targetDateSelector'],
        ]);
...
//	}
        $return .= '
        <label for="' . $option['name'] . '" class="inp ' . $option['direction'] . ' ' . $optioncols[0] . ' ' . $option['label-class'] . ' ">
                ';
...
//	}
  ...
  <span class="border"></span>
    ...
    static function field_time($options = [])
    {
        ...
        $return .= '
                <input type="text" data-offset="-50%" class="form-control w-100  ' . $option['direction'] . '" ' . $read . ' readonly name="' . $option['name'] . '"  id="' . $option['id'] . '"  value="' . $option['value'] . '"   ' . $requ . ' ' . $atts . ' >
    ';
  ...
  <span class="border"></span>
    ...
    static function periodToPeriod($from, $now)
    {

        $YMStart = self::date_int('Y,m', $from);
        $YMNow = self::date_int('Y,m', $now);

        $exStart = explode(',', $YMStart);
        ...
        for ($i = $exStartY; $i <= $exNowY; $i++) {
            if ($i == $exStartY) {
                $startMM = intval($exStartM);
            } else {
                $startMM = 1;
            }

            for ($j = $startMM; $j <= 12; $j++) {
                if ($i == $exNowY && $j > $exNowM) {
                    break;
                }
                $num_padded = sprintf("%02d", $j);
                $mNow = $i . $num_padded;
                array_push($dataDate, $mNow);
            }
        }
        $dataDate = arrayToKey($dataDate);

        return $dataDate;
    }

    static function periodLastDay($periodInput)
    {
        $dateFix = substr($periodInput, 0, 4) . '/' . substr($periodInput, 4, 2);
        $a_date = $dateFix . "/01";
        $dateToDate = self::datetodate($a_date, 3, 0, 3, 'en');
        return self::date_int("t", $dateToDate);
    }

    static function periodLastDayDate($periodInput)
    {
        $lDay = self::periodLastDay($periodInput);
        $dateFix = substr($periodInput, 0, 4) . '/' . substr($periodInput, 4, 2);
        return $dateFix . '/' . $lDay;
    }

    static function periodFirstDayDate($periodInput)
    {
        $dateFix = substr($periodInput, 0, 4) . '/' . substr($periodInput, 4, 2);
        return $dateFix . '/01';
    }
